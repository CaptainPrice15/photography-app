  ...
  if (first.result === 1022 && !code) {
    throw new Error(
      "pCloud requires a verification code (device verification or 2FA). " +
        "Set PCLOUD_CODE, or fetch a token once and set PCLOUD_AUTH_TOKEN."
    );
  }
  if (first.result !== 0) {
...
async function listFolder(folderId: string, token: string): Promise<PCloudFile[]> {
...
async function readManifestMeta(token: string): Promise<Map<string, ManifestMeta>> {
  const meta = new Map<string, ManifestMeta>();
  try {
    const listing = await listFolder(ROOT_FOLDER_ID, token);
    const manifest = listing.find((f) => !f.isfolder && f.name === "manifest.json");
...
async function getDownloadUrl(fileid: number, token: string): Promise<string> {

...
async function main(): Promise<void> {
  ...
  const photoMeta: Record<string, { width: number; height: number }> = {};
  let total = 0;
  let failed = 0;
  ...
  for (const dir of root.metadata?.contents ?? []) {
    ...
    const files = (listing.metadata?.contents ?? [])
      ...
      .map((f: any) => ({ name: f.name, fileid: f.fileid }))
    ...
    for (const file of files) {
      ...
      if (OPTIMIZABLE.has(ext)) {
        try {
          ...
          console.log(`  ✗ ${file.name} — ${err instanceof Error ? err.message : String(err)}`);
          failed++;
        }
      } else {
